...
- 新模块先在本地 Python package 内实现，不引入数据库迁移。
- `OrderLedger` 第一阶段可以先定义接口和内存实现，持久化 SQLite 版本在后续阶段引入。
- 交易 API 接入顺序建议先从 `DryRunExecutionAdapter` 或 `CCXTAdapter` 开始，因为验证成本最低。

## 11. 待用户确认

这些问题不阻塞第一阶段设计实现，但会影响后续阶段：

- 第一批真实 API 适配器优先接加密货币 `CCXT`，还是港股 `Longbridge`，还是 A 股 `QMT`。
- 是否需要支持多账户，或第一版只支持单账户。
- 是否需要把订单台账第一版直接持久化到 SQLite。
- 实盘运行模式优先做 CLI 手动触发，还是守护进程循环执行。
