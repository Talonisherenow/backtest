...
<body>
  ...
  <script>
    ...
    const state = {
      sourceId: payload.default_source_id || sources[0]?.source_id || "default",
      symbol: payload.default_symbol || sources[0]?.symbols?.[0]?.symbol || "",
      frequency: payload.default_frequency || sources[0]?.symbols?.[0]?.series?.[0]?.frequency || payload.frequency || "1d",
      board: "all",
      search: "",
    ...
    async function loadManifest() {
      ...
      try {
        ...
        state.search = "";
        searchInput.value = "";
        const requestedSymbol = payload.default_symbol || state.symbol;
        const symbols = currentSymbols();
        state.symbol = symbols.some((item) => item.symbol === requestedSymbol)
          ? requestedSymbol
          : symbols[0]?.symbol || "";
        const requestedFrequency = payload.default_frequency || state.frequency;
        const selectedSeries = seriesList(selectedItem());
        state.frequency = selectedSeries.some((entry) => entry.frequency === requestedFrequency)
          ? requestedFrequency
          : selectedSeries[0]?.frequency || manifest.frequency || "1d";
        state.seriesData = null;
        populateBoards();
        populateSymbols();
        renderDataStatus();
