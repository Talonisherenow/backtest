  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    @media (max-width: 900px) {
      .topline, main, .data-monitor { grid-template-columns: 1fr; }
      .header-actions { justify-items: start; }
      .metrics { min-width: 0; }
      .data-monitor { margin-left: 12px; margin-right: 12px; }
...
<body>
  ...
  <script>
    ...
    const shellDataApiBaseUrl = payload.data_api_base_url || "";
    ...
    function dataApiUrl(path) {
      const baseUrl = String(payload.data_api_base_url || shellDataApiBaseUrl || "").replace(/\\/+$/, "");
      return baseUrl ? `${baseUrl}${path}` : path;
    }

    function dataMonitorEnabled() {
      return Boolean(payload.data_api_base_url || shellDataApiBaseUrl);
    }
    ...
    async function loadPayload() {
      if (payload.mode === "dynamic") {
        ...
        try {
          ...
          payload = {
            ...apiPayload,
            data_api_base_url: apiPayload.data_api_base_url || shellDataApiBaseUrl,
            links: {
              ...shellLinks,
              ...(apiPayload.links || {}),
            },
