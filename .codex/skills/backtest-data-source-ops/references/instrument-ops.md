...
2. Confirm the exact `source_id` and inspect `provider_type`.
3. For one-off sync, call `POST /api/instrument-sync/run` with `{"source_id":"bitget"}` or `{"source_id":"a_share"}`.
...
- Instrument sync does not submit K-line jobs. Recurring bars need a separate
  data crawl schedule, typically `job.target.mode=tag` on the source default tag
  (`a_share` or `bitget`). See `schedule-ops.md`.
...
- If live A-share listings are required but `/api/instrument-sources` shows
  `provider_type=universe_csv`, restart data-source with
  `--a-share-catalog-source akshare` before relying on instrument sync.
